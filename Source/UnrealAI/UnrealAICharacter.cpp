...
void AUnrealAICharacter::Tick(float DeltaSeconds)
{
    ...
    if (CurrentPlayerVelocity.Size() > 0.0f)
    {
        PawnNoiseEmitter->MakeNoise(this, 1.0f, GetActorLocation());
    }
}

void AUnrealAICharacter::SetCurrentPlayerVelocityOnXAxis(float Velocity)
{
	CurrentPlayerVelocity.X = Velocity;
}

void AUnrealAICharacter::SetCurrentPlayerVelocityOnYAxis(float Velocity)
{
